...
@materialize("file://raw/extraction_metadata_log.json")
def write_extraction_metadata(data_path:Path, metadata):
    """
    ...
    with open("extraction_metadata_log.json", "w") as f:
        json.dump(metadata, f, indent=2)



@task(name="extract_data")
def extract_data(data_path: Path, dataset_name: str) -> pd.DataFrame:
    """
    ...
    metadata_path = data_path / "raw" / "extraction_metadata.json"
    write_extraction_metadata(metadata)
